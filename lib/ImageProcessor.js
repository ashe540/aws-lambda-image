...
class ImageProcessor {
    ...
    run(config) {
      if(!process.env.s3Bucket) {
        console.log('No bucket was provided via environmental variables!');
        return;
      }
      ...
      config.set("bucket", bucketName);

        return this.fileSystem.getObject(
            bucketName, // this.s3Object.bucket.name
    ...
    processImage(imageData, config) {
        ...
        var resizeCfg = bucket.replace('-dev', ''); //Remove dev for fetching config as it is common
        var cfg = config.get("resizes");

        cfg[resizeCfg].filter((resize) => {
            return resize.size &&
